...
import { fetchAnonymousStore, writeAnonymousStore } from '../cloud/anonymousCloud';

const POLL_INTERVAL_MS = 6000;

/**
 * Zero-setup shared backend for two-person use: a single anonymous
 * jsonblob.com document, no auth, polled instead of pushed (that
 * service has no realtime API). Same method surface as
 * LocalStorageRepository/FirestoreRepository so the swap in
 * repository.ts stays transparent to the rest of the app.
 */
export class AnonymousCloudRepository {
  ...
  constructor(private storeId: string, private onFatalError: (message: string) => void) {
    void this.poll(true);
    this.pollTimer = setInterval(() => void this.poll(false), POLL_INTERVAL_MS);
  }

  private async poll(isFirst: boolean): Promise<void> {
    if (this.writing) return; // avoid clobbering an in-flight local write with a stale read
    try {
      const remote = await fetchAnonymousStore(this.storeId);
      this.db = remote;
      this.ready = true;
      this.notify();
    } catch (err) {
      if (isFirst) this.onFatalError(err instanceof Error ? err.message : 'שגיאת חיבור לענן');
    }
  }
  ...
  private commit(next: DbShape): void {
    ...
    this.notify();
    this.writing = true;
    writeAnonymousStore(this.storeId, next)
      .catch((err: Error) => this.onFatalError(err.message))
      .finally(() => {
        this.writing = false;
      });
  }
